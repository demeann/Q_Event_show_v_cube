"""Стартовые пуши туров: после почты — интро R1, если тур ещё не завершён по БД; иначе по расписанию в окне тура."""
...
# Тексты совпадают с интро в хендлерах /play (R2 — формулировка для пуша / ТЗ).
TOUR_PUSH_R1_TEXT = (
...
TOUR_PUSH_R3_TEXT = (
    ...
    "мы добрые до конца.</tg-spoiler></i>"
)

# Совпадает с aiogram CallbackData.pack()
CB_R1_GO = "r1fwd:go"
CB_R2_GO = "r2go:go"
CB_R3_GO = "r3go:go"
...
async def _send_one_push(
    ...
    *,
    ...
    try:
        ...
        log.info("tour_push skip forbidden tg=%s round=%s", telegram_user_id, code.value)
        return True
    except TelegramBadRequest as e:
        log.warning("tour_push bad_request tg=%s round=%s %s", telegram_user_id, code.value, e)
        return False
    except Exception:
        log.exception("tour_push failed tg=%s round=%s", telegram_user_id, code.value)
        return False
        ...
        for rnd in rounds:
            ...
            for user in users:
                ok = await _send_one_push(
                    bot, telegram_user_id=user.telegram_user_id, code=rnd.code
                )
                if ok:
                    setattr(user, col.key, now_naive)
                    await session.flush()
