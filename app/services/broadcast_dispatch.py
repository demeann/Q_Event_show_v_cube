"""Отправка запланированных рассылок (Telegram) с обновлением статусов в БД."""
...
async def process_due_broadcasts(bot: Bot, session: AsyncSession | None = None) -> None:
    ...
    if session is None:
        async with get_session() as s:
            await _process_due_broadcasts_with_session(bot, s)
    else:
        await _process_due_broadcasts_with_session(bot, session)


async def _process_due_broadcasts_with_session(bot: Bot, session: AsyncSession) -> None:
    ...
    for rec in recipients:
        ...
        try:
            ...
            rec.error = f"forbidden:{e}"
            fail_c += 1
        except TelegramBadRequest as e:
            rec.status = RecipientStatus.FAILED
            rec.error = f"bad_request:{e}"
            fail_c += 1
