...
5. Запустить бота:
   ...
   Команда **`/play`** запускает **Тур 1**, если текущие дата/время попадают в окно тура в БД (задаётся `GAME_START_DATE_MSK` + сидинг `seed_content`). Вне окна бот сообщит, что тура нет.
